...
class ModemPlotter:
    ...
    def __init__(self, csv_file: Optional[str] = None, config: Optional[Dict] = None):
        ...
        self.z = None
        self.rho = None
        self.mt_stations = None  # Store MT station data
        self.loaded = False
        
        # Default configuration with extensive options
        self.config = {
            # DATA RANGES (in km) 
            'x_range_km': (-50.0, 50.0),      # Easting: -50 to 50 km
            'y_range_km': (-60.0, 60.0),      # Northing: -60 to 60 km
            'z_range_km': (0, 40.0),          # Depth: 0 to 40 km
            ...
            # DATA FORMAT SETTINGS (NEW)
            'data_in_natural_log': True,      # Set to True if data is in natural log
            'convert_to_log10': True,         # Convert to log10 for plotting
            
            # COLORBAR COMPACT SIZE
            'cbar_width': 0.3,                # Reduced width for compact colorbar
            'cbar_height': 0.015,             # Reduced height
            'cbar_y_position': 0.04,          # Lower position
            ...
            'save_format': 'png',
            'save_dpi': 300,
            'tight_layout': False,            # Disable for manual control
    ...
    def load_data(self, csv_file: str) -> 'ModemPlotter':
        ...
        try:
            ...
            if self.rho.min() < 0:  # Log values are often negative
                # Check if values look like natural log (typically larger magnitudes)
                median_val = np.median(self.rho)
                self.is_natural_log_data = median_val < -1  # Natural log values are more negative
    ...
    def _align_plot_to_axes(self, ax: plt.Axes, X: np.ndarray, Y: np.ndarray) -> None:
        """
        Ensure the pcolormesh plot exactly fills the axis limits.
        """
        try:
    ...
    def plot_horizontal(self, depth: float = 5000, ax: Optional[plt.Axes] = None, 
                       title: Optional[str] = None, plot_stations: Optional[bool] = None, **kwargs) -> Tuple[Optional[plt.Axes], Optional[Any]]:
    ...
    def plot_vertical(self, orientation: str = 'x', position: float = 0, 
                     ...
                     plot_stations: bool = False, **kwargs) -> Tuple[Optional[plt.Axes], Optional[Any]]:
        ...
        try:
            ...
            for idx, plot_config in enumerate(plots):
                ...
                if im is not None:
                    ...
                    is_last_row_in_incomplete_column = (nplots % ncols != 0 and 
                                                       idx >= nplots - (nplots % ncols))
                    
                    if is_bottom_row or is_last_row_in_incomplete_column:
...
if __name__ == "__main__":
    ...
    # Create a plotter (replace with your actual data file)
    plotter = ModemPlotter('output/gvb_inv5b_NLCG_130.csv', {
        ...
        'plot_mt_stations': False,  # DISABLE globally - default no stations
        'mt_station_color': 'white',
        'mt_station_marker': '^',
        'mt_station_size': 45,
        'mt_station_edgecolor': 'black',
    ...
    for i, depth in enumerate(depths):
        ...
        # Add stations only to the second plot (index 1, which is 5 km)
        if i == 1:  # Second plot (0-based index)
            plot_config['plot_stations'] = True  # Stations ONLY here
        else:
            plot_config['plot_stations'] = False  # No stations on other plots
